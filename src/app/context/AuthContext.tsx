/**
 * AuthContext.tsx
 * ユーザーの認証状態（サインイン・サインアウト）を管理する。
 *
 * ## 提供機能
 * - AuthProvider：アプリ全体に認証状態（user, isLoading, signIn, signOut, setUser）を共有するコンポーネント
...
export const AuthProvider = ({ children }: { children: ReactNode }) => {
  ...
  useEffect(() => {
    ...
    if (!isLoading && user === null) {
      // 現在のパスが/signinでなければリダイレクトする（無限ループ防止）
  ...
  const signIn = async (email: string, password: string) => {
    ...
    // 認証成功 → ユーザー情報をstateに保存（サインイン状態にする）
    ...
    // Cookieに保存することで、リロード時にも状態を維持できるようにする
  ...
  return <AuthContext.Provider value={{ user, isLoading, signIn, signOut, setUser, deleteAccount }}>{children}</AuthContext.Provider>;
};
