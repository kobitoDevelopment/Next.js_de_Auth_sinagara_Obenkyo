...
export async function signIn(formData: FormData): Promise<SignInResult> {
  ...
  if (email && password) {
    // メールアドレスでユーザーを検索
    const { data, error } = await supabase.from("users").select("*").eq("email", validEmail).single();
    user = data;
 ...
 */
export const signInAction = async (prevState: SignInResult, formData: FormData): Promise<SignInResult> => {
