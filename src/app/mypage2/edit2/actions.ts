...
export async function updateProfile(formData: FormData): Promise<UpdateProfileResult> {
  ...
  if (userId && typeof email === 'string') {
    const { data: _emailUsers, error: emailCheckError } = await supabase
      .from('users')
      .select('id')
      .eq('email', email)
      .neq('id', userId);
  ...
  if (userId && typeof username === 'string') {
    const { data: _usernameUsers, error: usernameCheckError } = await supabase
      .from('users')
      .select('id')
  ...
  if (userId) {
    ...
    if (updateError) {
      ...
      if (
        updateError.details &&
        typeof updateError.details === 'string' &&
        updateError.details.trim() !== ''
      ) {
