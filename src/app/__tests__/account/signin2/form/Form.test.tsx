...
describe("signIn", () => {
  it("正常なサインイン処理が成功する", async () => {
    ...
    // Cookieが正しくセットされる
    expect((cookies as jest.Mock)().set).toHaveBeenCalledWith("user_id", "user1", expect.any(Object));
  });
  ...
  it("バリデーションエラーが複数返る（メール形式・パスワード長）", async () => {
    ...
    // 複数のバリデーションエラーが配列で返る
    expect(result).toEqual({
      errors: expect.arrayContaining(["有効なメールアドレスを入力してください", "パスワードは6文字以上で入力してください"]),
    });
  ...
  it("バリデーションエラーとDBエラーが両方返る（複数エラー）", async () => {
    ...
    // 両方のエラーが配列で返る
    expect(result).toEqual({
      errors: expect.arrayContaining(["パスワードは6文字以上で入力してください", "ユーザーが見つかりません"]),
    });
    expect(redirect).not.toHaveBeenCalled();
  });
});
