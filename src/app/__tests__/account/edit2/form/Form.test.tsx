import { updateProfile } from "@/app/edit2/actions";
...
describe("updateProfile", () => {
  it("正常系：ユーザー名・メール更新のみ", async () => {
    ...
    mockNeq.mockResolvedValueOnce({ data: [], error: null }).mockResolvedValueOnce({ data: [], error: null });
    mockUpdateEq.mockResolvedValue({ error: null });
  ...
  it("バリデーションエラーが複数返る（ユーザー名必須・メール形式・パスワード長）", async () => {
    ...
    expect(result?.errors).toEqual(expect.arrayContaining(["ユーザー名は必須です", "有効なメールアドレスを入力してください", "パスワードは6文字以上で入力してください"]));
  ...
  it("メールアドレス・ユーザー名・パスワード長がすべて重複・エラーの場合", async () => {
    ...
    mockNeq.mockResolvedValueOnce({ data: [{ id: "other-user" }], error: null }).mockResolvedValueOnce({ data: [{ id: "other-user2" }], error: null });
    ...
    expect(result?.errors).toEqual(expect.arrayContaining(["このメールアドレスは既に登録されています", "このユーザー名は既に登録されています", "パスワードは6文字以上で入力してください"]));
    expect(mockUpdate).not.toHaveBeenCalled();
  });
  ...
  it("新パスワード入力時に現在のパスワード未入力ならエラー", async () => {
    mockSingle.mockResolvedValueOnce({
      data: { id: "user-id-1", password: "hashedpass", username: "username", email: "mail@example.com" },
      error: null,
    });
    mockNeq.mockResolvedValueOnce({ data: [], error: null }).mockResolvedValueOnce({ data: [], error: null });
  ...
  it("型エラー: フォーム値がstring以外の場合、フォームの入力が不正です", async () => {
    const formData = {
      get: (key: string) => (key === "username" ? 123 : key === "email" ? "mail@example.com" : key === "current_password" ? "" : key === "new_password" ? "" : null),
    } as unknown as FormData;
