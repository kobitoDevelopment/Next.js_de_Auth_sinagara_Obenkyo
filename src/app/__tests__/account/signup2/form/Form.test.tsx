...
describe("registerUser", () => {
  ...
  it("バリデーションエラーが全て返される", async () => {
    ...
    expect(result).toEqual({
      errors: expect.arrayContaining(["ユーザー名は必須です", "メールアドレスの形式が正しくありません", "ロールは必須です", "パスワードは6文字以上で入力してください"]),
    });
  ...
  it("メールアドレスもユーザー名も重複している場合、両方のエラーが返る", async () => {
    ...
    // 両方の重複エラーが配列で返る
    expect(result).toEqual({
      errors: expect.arrayContaining(["このメールアドレスは既に登録されています", "このユーザー名は既に登録されています"]),
    });
    expect(mockInsert).not.toHaveBeenCalled();
    expect(redirect).not.toHaveBeenCalled();
  });
