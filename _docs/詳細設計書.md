...
本プロジェクトはNext.jsとSupabaseを用いた認証機能を中心とした学習プロジェクトである。Server Actionsを活用した実装手法を学び、型安全なアプリケーション開発を目指す。
...
export async function registerUser(formData: FormData): Promise<RegisterResult> {
  ...
  // 4. ユーザー名・メールアドレスの重複チェック（並列処理）
  // 5. パスワードのbcryptハッシュ化
  // 6. Supabaseへのユーザー登録
  // 7. 結果に基づくエラー返却またはリダイレクト
}
